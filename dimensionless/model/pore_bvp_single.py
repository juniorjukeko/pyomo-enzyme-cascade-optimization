...
def add_bvp_constraints(model, decay_coef={'kA':0, 'kB':0}, bvp_kwargs=None): 
    ...
    def typeA_S1_pde_rule(m, xi, t): # S1 is CONSUMED here
        lhs_mult, rhs_rate = get_enzyme_kinetics(
            m.EA_x_profile[xi], m.decay_A[t], m.kA, m.u_n_A['S1', xi, t],
            S_ref=S_ref, Km=Km_A, kinetics_type=kinetics_type_A
        )
        return  lhs_mult * (m.D['S1'] / m.L**2) * m.d2u_ndxi2_A['S1', xi, t] == rhs_rate
    model.A_S1_pde = pyo.Constraint(model.xi, model.time, rule=typeA_S1_pde_rule)

    def typeA_S2_pde_rule(m, xi, t): # S2 is GENERATED here
        lhs_mult, rhs_rate = get_enzyme_kinetics(
            m.EA_x_profile[xi], m.decay_A[t], m.kA, m.u_n_A['S1', xi, t],
            S_ref=S_ref, Km=Km_A, kinetics_type=kinetics_type_A
        )
        return  lhs_mult * (m.D['S2'] / m.L**2) * m.d2u_ndxi2_A['S2', xi, t] == -rhs_rate
    model.A_S2_pde = pyo.Constraint(model.xi, model.time, rule=typeA_S2_pde_rule)

    def typeA_S3_pde_rule(m, xi, t): # S3 does nothing but diffuse
        return (m.D['S3'] / m.L**2) * m.d2u_ndxi2_A['S3', xi, t] == 0
    model.A_S3_pde = pyo.Constraint(model.xi, model.time, rule=typeA_S3_pde_rule)

    # --- 4b. BVP: Enzyme B only: S2 -> S3 ---
    def typeB_S1_pde_rule(m, xi, t): # S1 does nothing but diffuse
        return (m.D['S1'] / m.L**2) * m.d2u_ndxi2_B['S1', xi, t] == 0
    model.B_S1_pde = pyo.Constraint(model.xi, model.time, rule=typeB_S1_pde_rule)

    def typeB_S2_pde_rule(m, xi, t): # S2 is CONSUMED here
        lhs_mult, rhs_rate = get_enzyme_kinetics(
            m.EB_x_profile[xi], m.decay_B[t], m.kB, m.u_n_B['S2', xi, t],
            S_ref=S_ref, Km=Km_B, kinetics_type=kinetics_type_B
        )
        return lhs_mult * (m.D['S2'] / m.L**2) * m.d2u_ndxi2_B['S2', xi, t] == rhs_rate
    model.B_S2_pde = pyo.Constraint(model.xi, model.time, rule=typeB_S2_pde_rule)

    def typeB_S3_pde_rule(m, xi, t): # S3 is GENERATED here
        lhs_mult, rhs_rate = get_enzyme_kinetics(
            m.EB_x_profile[xi], m.decay_B[t], m.kB, m.u_n_B['S2', xi, t],
            S_ref=S_ref, Km=Km_B, kinetics_type=kinetics_type_B
        )
        return lhs_mult * (m.D['S3'] / m.L**2) * m.d2u_ndxi2_B['S3', xi, t] == -rhs_rate
    model.B_S3_pde = pyo.Constraint(model.xi, model.time, rule=typeB_S3_pde_rule)

    # --- 5. Boundary conditions & Flux Rules (Remain the same) --- 
    def bc1_A_rule(m, component, t):
        return m.u_n_A[component, m.xi.first(), t] == m.u_0[component, t]
    model.bc1_A = pyo.Constraint(model.Components, model.time, rule=bc1_A_rule)

    def bc1_B_rule(m, component, t):
        return m.u_n_B[component, m.xi.first(), t] == m.u_0[component, t]
    model.bc1_B = pyo.Constraint(model.Components, model.time, rule=bc1_B_rule)

    def bc2_A_rule(m, component, t):
        return m.du_ndxi_A[component, m.xi.last(), t] == 0
    model.bc2_A = pyo.Constraint(model.Components, model.time, rule=bc2_A_rule)

    def bc2_B_rule(m, component, t):
        return m.du_ndxi_B[component, m.xi.last(), t] == 0
    model.bc2_B = pyo.Constraint(model.Components, model.time, rule=bc2_B_rule)

    def flux_rule(m, component, t):
        Np_A = m.Np / 2  
        Np_B = m.Np / 2  
        flux_A = -m.D[component] * (m.S_ref / m.L) * m.du_ndxi_A[component, m.xi.first(), t] * m.A * Np_A
        flux_B = -m.D[component] * (m.S_ref / m.L) * m.du_ndxi_B[component, m.xi.first(), t] * m.A * Np_B
